...
def change_status():    
    try:
        ...
        change_status = Todos_db.change_status(uid, status)
...
def all_todos():
    try:
        uid = get_jwt_identity()
        all_todos = convert_to_json_serializable(list(Todos_db.get_todos_by_user_id(uid).get("todo", {})))
        
        return jsonify({"todos": all_todos, 'status': 'success'})
